...
class TransactionDetailActivity : BaseMvpActivity<TransactionDetailView, TransactionDetailPresenter>(),
    ...
    override fun setTransactionType(type: Direction, isFeeTransaction: Boolean) {
        transaction_type.text = resources.getString(
            when (type) {
                Direction.TRANSFERRED -> R.string.tx_moved
                Direction.RECEIVED -> R.string.tx_received
                Direction.SENT -> if (isFeeTransaction)
                    R.string.pax_fee_1
                else
                    R.string.tx_sent
                Direction.BUY -> R.string.tx_bought
                Direction.SELL -> R.string.tx_sold
                Direction.SWAP -> R.string.tx_swapped
            }
        )
    }
