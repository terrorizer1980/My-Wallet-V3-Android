...
sealed class BitPayEvent(override val event: String, override val params: Map<String, String>) : AnalyticsEvent {
    ...
    data class SuccessEvent(val amount: BigInteger) :
        BitPayEvent(AnalyticsEvents.BitpayPaymentSucceed.event, mapOf("amount" to amount.toString()))

    data class FailureEvent(val message: String) :
        BitPayEvent(AnalyticsEvents.BitpayPaymentFailed.event, mapOf("error" to message))
