...
public class VeriffStatusReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (extras.containsKey(VeriffConstants.INTENT_EXTRA_STATUS)) {
            ...
            } else if (status == VeriffConstants.STATUS_UNABLE_TO_RECORD_AUDIO) {
                //Veriff broadcast: audio permission missing
                Toast.makeText(context, "Veriff broadcast: audio permission missing", Toast.LENGTH_LONG).show();
            ...
            } else if (status == VeriffConstants.STATUS_OUT_OF_BUSINESS_HOURS) {
                //Veriff broadcast: business hours
                Toast.makeText(context, "Veriff broadcast: out of business hours", Toast.LENGTH_LONG).show();
            ...
            } else if (status == VeriffConstants.STATUS_VIDEO_CALL_ENDED) {
                //Veriff broadcast: videocall ended successfully
                Toast.makeText(context, "Veriff broadcast: video call ended successfully", Toast.LENGTH_LONG).show();
            }
        }
    }
}
