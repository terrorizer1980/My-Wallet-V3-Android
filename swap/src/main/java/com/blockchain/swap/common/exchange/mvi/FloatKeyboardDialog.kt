    ...
    val states: Observable<FloatEntryViewState> =
        intents.scan(initialState) { previous, intent ->
            when (intent) {
                ...
                is FloatKeyboardIntent.Period -> mapPeriodPress(
                    previous
                )
                is FloatKeyboardIntent.NumericKey -> mapKeyPress(
                    previous,
                    intent.key
                )
...
private fun numberToIntents(intent: FloatKeyboardIntent.SetValue): List<FloatKeyboardIntent> {
    ...
    val integerIntents = listOf(
        FloatKeyboardIntent.SetMaxDp(
            intent.maxDp
        )
    ) + intToIntents(integer, false)
    val onePlusFractionIntents =
        intToIntents(onePlusFraction, true)
    ...
    return when {
        ...
        if (previous.maximums.maxIntLength in 1..(integerDigits - 1)) {
            ...
            constructViewStateFromValue(
                previous,
                previous,
                FloatKeyboardIntent.SetValue(
                    previous.maxDecimal,
                    previous.maximums.maxValue
                )
            ).shake()
        } else {
            new
