...
import com.blockchain.swap.shapeshift.data.Quote
import com.blockchain.swap.shapeshift.data.State
...
class ShapeShiftDataManagerTest {
    ...
    fun `findTrade found`() {
        ...
        val tradeData: ShapeShiftTrades = mock()
        val trade = Trade()
            .apply { quote = Quote().apply { deposit = depositAddress } }
    ...
    fun `updateTrade found, save successful`() {
        ...
        val trade = Trade()
            .apply { quote = Quote().apply { this.orderId = orderId } }
    ...
    fun `updateTrade not found`() {
        ...
        val trade = Trade()
            .apply { quote = Quote().apply { this.orderId = orderId } }
        val updatedTrade = Trade()
            .apply { quote = Quote().apply { this.orderId = "" } }
        val list = mutableListOf(trade)
        val tradeData: ShapeShiftTrades = mock()
        whenever(shapeShiftDataStore.tradeData).thenReturn(tradeData)
        whenever(tradeData.trades).thenReturn(list)
